...
function checkReportOrParse(path, res) {
    fs.stat(path, function(err, stat) {
        if(err == null) {
            ...
            const ls = exec('junit2html ' + path, function (error, stdout, stderr) {
                if (error) {
...
app.get('/api/failed-tests/:fileName', function(req, res, next){
    (async () => {
        ...
        await Promise.all(failedTests.map(async test => {
                ...
                let stdout = await page.evaluate((id) => document.getElementById(id).parentElement.getElementsByTagName('pre')[0].outerHTML, id);
                response.push({id : id, text: text, stdout: stdout})
            })
        )
        res.send(response)
...
app.get('/xml/:filename', function(req, res, next){
    fs.access(dst + req.params.filename, fs.constants.F_OK, (err) => {
        err ? console.log(`${req.params.filename} does not exist`) : checkReportOrParse(dst + req.params.filename + '.html', res)
    })
});
